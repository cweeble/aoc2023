...
my @starts = grep {/..A/} keys %map;
my @final;
while (my $startingPos = shift @starts) {
    my $currentPos = $startingPos;
    my $answer;
    INFINITE: while (1) {
        ...
        for my $way (@directions) {
            ...
            last INFINITE if ($currentPos =~ /..Z/);
        }
    }
    print "Starting at ${startingPos} Answer=${answer}\n";
    push @final, $answer;
}

# LCM as each is an exact multiple of the 269 steps.
my @iterations;
map { push @iterations, $_/length($directions)} @final;
my $answer = length($directions);
map { $answer *= $_ } @iterations;
