...
while (my $line = <DATA>) {
    ...
    my @finalReds = sort {$b <=> $a} (@reds);
    my @finalGreens = sort {$b <=> $a} (@greens);
    my @finalBlues = sort {$b <=> $a} (@blues);
    
    print "Game ${game}: ";
    my $ok = 1;
    my ($minR, $minG, $minB) = ($finalReds[0], $finalGreens[0], $finalBlues[0]);
    print join(':', $minR, $minG, $minB);
    my $pwr = $minR * $minG * $minB;
    print " Pwr=${pwr} ";
    $total += $pwr;
