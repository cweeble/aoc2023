...
our @rows = (<DATA>);
{
    my ($row0, $row1) = ($rows[0], $rows[1]);
    if ($row1 =~ /12345678901234567890/) {
        shift @rows for (1..2);
    }
}
our @rowHashes;
...
for my $row (@rows) {
    $rownum++;
    my %columns;
    my @numbers = split(/\D+/, $row); #list of all numbers in each row
    shift @numbers unless ($numbers[0] =~ /\d/);
    my $inNumber = 0;
    my @cols = split(//, $row);
    my $cols = -1;
    for my $c (@cols) {
        ...
        if ($c =~ /\d/) {
            $inNumber = 1;
            $columns{$cols} = $numbers[0];
        } else {
            shift @numbers if ($inNumber);
            $inNumber = 0;
        }
    }
    push (@rowHashes, \%columns);
}
...
for my $row (@rows) {
    ...
    for my $c (@rowChars) {
        $col++;
        next if ($c =~ /[0-9\.]/);
        my $west = West($col, $rowNum);
        my $east = East($col, $rowNum);

        my @north = North($col, $rowNum);

        my @south = South($col, $rowNum);

#       print "col:row:symbol = " . join(':', $col, $rowNum, $c);
#       print " NSEW = " . join(':', join(',',@north), join(',', @south), $east, $west);
        map { $total += $_ } (@north, @south, $east, $west);
#       print " total = ${total}\n";

        if ($c eq '*') {
            my $count = grep { defined $_ } (@north, @south, $east, $west);
            print "\ncol:row:symbol:count = " . join(':', $col, $rowNum, $c, $count);
            if ($count == 2) {
                my $gearRatio = 1;
                for my $mult (@north, @south, $east, $west) {
                    $gearRatio *= $mult if ($mult);
                }
                $gearTotal += $gearRatio;
            }
        }
...
sub North {
    my ($x, $y) = @_; #x=thisCol, y=thisRow
    return undef if ($y-- == 0 ); #no row -1
    my %hash = % {$rowHashes[$y]};
    my $answer = $hash{$x};
    if ($answer) {
        return ($answer);
    }
    my ($east, $west) = (West($x,$y), East($x,$y));
    my @answer;
    push (@answer, $west) if ($west);
    push (@answer, $east) if ($east);
    return @answer;
}
...
01234567890123456789012345678901234567890123456789012345678901234567890123456789012345679012345678901234567890123456789012345678901234567890
